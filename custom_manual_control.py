...
def check_differences(dir1, dir2):
    pathnames = glob.glob(dir1+'*')
    filenames_dir1 = [os.path.basename(path) for path in pathnames]
    pathnames = glob.glob(dir2+'*')
    filenames_dir2 = [os.path.basename(path) for path in pathnames]

    diff_dir1 = np.setdiff1d(filenames_dir1, filenames_dir2)
    diff_dir2 = np.setdiff1d(filenames_dir2, filenames_dir1)
...
class CameraManager(object):
    ...
    def set_sensor(self, index, notify=True, force_respawn=False):
        ...
        self.index = index
        if (index == 0 or force_respawn) and self.reserve_sensor is not None:
            self.reserve_sensor.destroy()
            self.reserve_sensor = None
        if index !=0 and self.reserve_sensor is None:
            self.reserve_sensor = self._parent.get_world().spawn_actor(
                self.sensors[0][-1],
                self._camera_transforms[self.transform_index][0],
                attach_to=self._parent,
                attachment_type=self._camera_transforms[self.transform_index][1])
            self.reserve_sensor.listen(lambda image: CameraManager._parse_image(weak_self, image, reserve_sensor=True))
    ...
    def toggle_recording(self):
        ...
        if not self.recording:
            sleep(1)
            #print('apagar')
            # if recording was turned off
            # clean the images that are not on the rgb side
            #print(self.sensors[self.index][2])
            #print(dir_dict[self.sensors[self.index][2]])
            check_differences(dir_dict[self.sensors[self.index][2]], dir_dict['Camera RGB'])
    ...
    @staticmethod
    def _parse_image(weak_self, image, reserve_sensor=False):
        
        self = weak_self()
        if not self:
            return
        #print(self.index)
        #print(self.sensors[self.index])
        #print()
        if self.sensors[self.index][0].startswith('sensor.lidar'):
            ...
            self.surface = pygame.surfarray.make_surface(array.swapaxes(0, 1))
        if self.recording:
            #print('_out/%08d' % image.frame)
            #sensor = self.sensors[self.index][2]
            if reserve_sensor:
                image.save_to_disk(dir_dict['Camera RGB']+'%08d' % image.frame)
            else:
                image.save_to_disk(dir_dict[self.sensors[self.index][2]]+'%08d' % image.frame)
